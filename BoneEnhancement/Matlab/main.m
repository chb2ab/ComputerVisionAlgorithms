...
imwrite(uint8(hlder.*255/max(max(hlder))), '1original.png')
...
imwrite(uint8(blurred.*255/max(max(hlder))), '2blurred.png')
...
Gy = -1.*Gy;
imwrite(uint8(Gy.*255/max(max(Gy))),'3differential.png')

%% Reflection Image
reflec = Gy+blurred; % reflec is the reflection number of the image
imwrite(uint8(reflec.*255/max(max(reflec))),'4reflection numbers.png')
...
imwrite(uint8(shadow.*255/max(max(shadow))),'5ShadowImage.png');

%% Probability image
probabi = shadow.*reflec; % probabi is the probability image for bones
probabi = probabi./max(max(abs(probabi)));
imwrite(uint8(probabi.*255),'6ProbabilityImage.png');
...
[val, ro] = max(probabi);
% start out not in a bone region
bone = false;
...
imwrite(hlder,'7Thresholded.png');

%% Thresholding Snake
% This was my attempt at the thresholding algorithm described in the paper,
% I couldn't finish it but I thought I'd include it to show how I was going
% about it.
ascale = 2;
gamma = -1;
jumpcost = 0.2;
...
D1 = 1;
D2 = 1;
alpha = (ascale)*(1/1225); % alpha and beta scaled so that the external energy is around 0-1.
...
for x = 1:1:length(cind)
    if cind(x) ~= 0
        hlder(cind(x),x,2) = 0;
        hlder(cind(x),x,1) = 0;
        hlder(cind(x),x,3) = 255;
    end
end
imwrite(hlder,'8SnakeThresh.png');
